...
class ClasspathScanner(private val classpath: String) {
    ...
    private fun collectIncludeAnnotations() {
        ...
        for (classInfo in scanResult.getClassesWithAnnotation("kaffa.portrait.Reflective\$Include")) {
            val annotations = classInfo.getAnnotationInfoRepeatable("kaffa.portrait.Reflective\$Include")
        ...
        for (classInfo in scanResult.getClassesWithAnnotation("kaffa.portrait.ProxyTarget\$Include")) {
            val annotations = classInfo.getAnnotationInfoRepeatable("kaffa.portrait.ProxyTarget\$Include")
            for (annotation in annotations) {
                val classesArray = annotation?.parameterValues?.get("classes")?.value as? Array<*> ?: continue
                val includes = extractIncludes(annotation.parameterValues?.get("including")?.value)
    ...
    private fun collectReflectives() {
        for (classInfo in scanResult.getClassesWithAnnotation("kaffa.portrait.Reflective")) {
            reflectives.add(classInfo)
            val annotation = classInfo.getAnnotationInfo("kaffa.portrait.Reflective")
            val includes = extractIncludes(annotation?.parameterValues?.get("including")?.value)
    ...
    private fun collectProxyTargets() {
        for (classInfo in scanResult.getClassesWithAnnotation("kaffa.portrait.ProxyTarget")) {
            proxyTargets.add(classInfo)
            val annotation = classInfo.getAnnotationInfo("kaffa.portrait.ProxyTarget")
            val includes = extractIncludes(annotation?.parameterValues?.get("including")?.value)
